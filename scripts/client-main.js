async function displayRecentPurchases() {
    const postDetails = {
        method: 'POST',
        headers: {
    ...
    const postResponse = await fetch('/this-users-recent-purchases', postDetails);
    const jsonData = await postResponse.json();

    let recentPurchases = jsonData.thisUsersPurchases;

    // Get each purchase's data
    for (let index = 0; index < recentPurchases.length; index++) {
        let currPurchase = recentPurchases[index];

        let purchaseTemplate = document.getElementById("purchaseTemplate");
        let recentPurchasesDiv = document.getElementById("recentPurchasesDiv");
        ...
        recentPurchasesDiv.appendChild(purchaseDiv);
    }
